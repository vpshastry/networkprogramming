...
dg_send_recv(int fd, int filefd)
{
  ...
  while (!lastloop) {
    ...
    if (sigsetjmp(jmpbuf2, 1) != 0) {
      ...
      if (window_probe_timer > 60)
        window_probe_timer = 60; // following TCP guidelines, could be lesser if needed.
...
sendagain:
    ...
    mysetitimer(&sa, &timer, rtt_start(&rttinfo));
    ...
    for (i = window->tail; i <= window->head; ) {
      ...
      mysetitimer(&sa,&timer, rtt_start(&rttinfo));/* calc timeout value & start timer */
